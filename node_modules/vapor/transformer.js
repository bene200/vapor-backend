...
Transformer.prototype.blastSearch = function(blastproxy, success){
    var Quickblast = require("../lib/quickblast/lib/blast"),
        fasta = "",
        lines = [],
        shortFasta = "";
...
Transformer.prototype.runClustal = function(galaxyInfo, success){
    var Galaxy = require("../lib/galaxyjs/lib/galaxy"),
...
Transformer.prototype.idsFromFastaSimple = function(fasta){
    var headers = fasta.match(/>.*/g),
        _ = require("underscore");
    return _.map(headers, function(el){
...
Transformer.prototype.getSTRINGNetworks = function(geneList, proxy, success){
    var StringDB = require("/Users/bene1/masters_thesis/biojs-stringdb/js/string"),
        sdb = new StringDB({ format: "psi-mi" }),
        net = {},
        networks = [],
        id = "";
    ...
    asyncLoop({
        ...
        functionToLoop : function(loop, i){
            ...
            sdb.getNetwork(id, proxy).then(function(res){
                if(res.substring(0,14) === "<!DOCTYPE html"){
                    loop();
                }
                else {
                    sdb.xmlNetworkToJSON(res, function(g){
                        networks.push({ query: id, graph: g });
                        loop();
                    });
                }
            });
...
Transformer.prototype.getGeneInfo = function(ids, proxy, done){
    var UniprotGet = require("./uniprot"),
        ...
        uniprotData = [],
        bar = new ProgressBar(':bar', { total: 20 }),
        part = ids.length/20;
